...
export async function getRecentEntries(yearMonth: string): Promise<EntriesResult> {
  ...
  const data = await fetchAllRows<RecentRow>(({ from, to }) =>
    supabase
      ...
      // An import writes hundreds of rows with one created_at; see fetchAllRows.
      .order("id")
      .range(from, to),
  );
...
export async function getMonthlySummary(yearMonth: string): Promise<{
  ...
  /** CapEx and Tax, shown separately and never subtracted from operating profit. */
  nonOperating: MonthlySummaryGroup[];
  totalRevenue: number;
  /**
   * Renamed from totalExpense deliberately. Redefining a name in place would
   * have silently changed the meaning of every existing consumer with nothing
   * failing to compile — the same shape as a ratio-vs-percentage mixup. The
   * rename makes the compiler find all of them.
   */

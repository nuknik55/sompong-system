...
import { thDate, locationLabel, fmtBaht, STATUS_LABEL } from "../shared";
...
export default async function CateringStatusPage({
  ...
  // urgent is only visible by scrolling past everything else in that bucket.
  ...
  const buckets = PIPELINE_STATUSES.map((status) => ({
    status,
    events: events.filter((e) => e.status === status && !upcomingIds.has(e.id)),
  }));
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-6 sm:px-6">
      ...
      {buckets.map((b) => (
        <PipelineSection key={b.status} title={STATUS_LABEL[b.status] ?? b.status} events={b.events} />
      ))}

      <div className="pt-1 text-xs">
        {includeHistory ? (
          <Link href="/owner/catering/status" className="text-neutral-400 hover:text-neutral-700">ซ่อนประวัติ (เสร็จสิ้น/ยกเลิก)</Link>
        ) : (
          <Link href="/owner/catering/status?history=1" className="text-neutral-400 hover:text-neutral-700">ดูประวัติ (เสร็จสิ้น/ยกเลิก) →</Link>
        )}
      </div>

      {includeHistory && (
        <PipelineSection title="ประวัติ (เสร็จสิ้น/ยกเลิก)" events={history} muted />
      )}
    </div>
  );
}
    ...
    >
      ...
      {events.length === 0 ? (
        ...
        <div className="divide-y divide-neutral-100">
          {events.map((e) => <PipelineRow key={e.id} event={e} />)}
        </div>
      )}
    </section>
  );
...
function PipelineRow({ event: e }: { event: CateringEvent }) {

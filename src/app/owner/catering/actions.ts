...
// The booking's food-line types live with their reads in menu-read.ts.
export type { CateringEventMenu, CateringSetMenuItem } from "./menu-read";
...
// The reads live in menu-read.ts (server-only, nothing that computes cost in
// its imports), so the menu card can call them without importing this file.
// These wrappers keep every other caller where it was: one body, one answer.

export async function getCateringEventMenus(eventId: string): Promise<CateringEventMenu[]> {
  return readEventMenus(eventId);
}
...
/** The shared sets' rows for many sets at once, behind requireSales(): see readSetMenuItemsForSets in menu-read.ts. */
export async function getCateringSetMenuItemsForSets(setMenuIds: string[]): Promise<Map<string, CateringSetMenuItem[]>> {
  return readSetMenuItemsForSets(setMenuIds);
}
...
/** What each set line of a booking serves; every screen that expands a set reads this (readEventMenuDishes in menu-read.ts). */
export async function getEventMenuDishes(eventId: string): Promise<Map<string, { source: DishSource; dishes: EventMenuDish[] }>> {
  return readEventMenuDishes(eventId);
}

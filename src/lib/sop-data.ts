...
export async function getSopList(): Promise<SopListItem[]> {
  ...
  const [{ data: menus }, { data: sops }] = await Promise.all([
    supabase.from("menus").select("id, name, category").order("name"),
    supabase.from("menu_sops").select("id, menu_id, updated_at, author_name"),
  ]);

  const sopByMenuId = new Map<string, { id: string; updated_at: string; author_name: string | null }>();

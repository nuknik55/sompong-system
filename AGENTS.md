...
| `public.order_review_approve(session, seen_version, lines)` | a head approves a waiting order WITH per-line quantities in ONE transaction (lines checked first, each change logged, stale version refused before any write); what the review screen calls | — | `order_review_approve_migration.sql` (written 2026-09-23, NOT APPLIED) |

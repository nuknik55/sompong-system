...
| `public.catering_save_set_draft(id, seen, name, price, items)` | **owner, admin** — a trial set's (is_draft) name, price and dishes in ONE transaction, the set row locked, refused when its updated_at is not `seen` (HINT 'conflict') and for a real set. SECURITY INVOKER | — | `catering_event_sheet_and_set_drafts_migration.sql` (**NOT applied**) |

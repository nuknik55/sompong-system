...
# A view is written through as its OWNER (the review, 2026-09-26)

Supabase grants signed-in accounts ALL on every new table AND VIEW by
default. A plain view over one table is updatable, and a write through it
runs as the view's owner, past the table's own policies: `ingredient_costs`,
made to HIDE prices, let any editor SET them. A GRANT only adds; it never
takes back what the default gave. So every new view gets `REVOKE ALL ...
FROM PUBLIC, anon, authenticated` and then `GRANT SELECT`, and every new
table the same before its grants. And a stand-in models the default
privileges (`ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES
TO anon, authenticated, service_role`): without them the stand-in passed a
file that would have stopped live.
...
| `public.can_see_cost()` | owner, admin, and an editor whose เห็นต้นทุน switch is on (a row in `profile_cost_access`; the owner alone writes it, editors only; a role change away from editor deletes it, trigger `profile_cost_access_clear`). Decides the view `ingredient_costs`, `ingredient_price_history_select`, `pos_receipt_deliveries_select` and `prep_unit_costs()`. App mirror: `seesCost()` in `src/lib/cost-access.ts` | — | `sop_visibility_and_editor_cost_switch_migration.sql` (applied 2026-09-26) |
| `public.can_see_sop(id)` | owner, admin: every SOP; anyone else with a profile: an SOP whose `visibility` is 'all', or one they are chosen for (`menu_sop_viewers`). RESTRICTIVE policies on `menu_sops` (select, update, delete: `sop_row_visible(id, visibility)`, the same rule on the row's own values, so an upsert's new row is judged by what it says), `menu_sop_steps` and `menu_sop_ingredient_notes` (all). Only owner and admin set visibility (trigger `menu_sops_visibility_guard`; `sop_set_visibility()` the one save) or delete a restricted SOP; no session moves a step, note or SOP. An SOP request follows the SOP (`sop_request_allowed`, restrictive filing and read policies on `pending_changes`). App mirror for approvals: `sopVisibleTo()` in `src/lib/sop-visibility.ts` | — | the same file (applied 2026-09-26) |
| `public.sop_doc_readable(id, visibility, kind, prep)`, `sop_doc_writable(..., category)`, `can_write_sop(id)`; `can_see_sop(id)` REPLACED to ask `sop_doc_readable` | read: a prep SOP owner, admin and editors who can see THE PREP (admins by grant), never staff, then its own list; a menu SOP or manual as `can_see_sop` above. write: what one reads, and then a menu or prep SOP owner, admin, editor; a manual its category's writers (`manual_category_writable(cat)`: owner, admin; editors where `editors_write`; the chosen `manual_category_writers`). `writes_some_manual()` lets writers upload. `sop_viewer_allowed(sop, profile)`: a prep SOP's list takes editors only. The chosen lists' policies ask `can_see_sop`. `sop_row_visible` DROPPED | — | `prep_sops_manuals_and_private_photos_migration.sql` (`a83554b`), **written, NOT applied** |
| `public.sop_doc_save(id, kind, prep, category, title, author, updated, video, steps, notes)` | a prep SOP or a manual, whole, in one transaction; SECURITY INVOKER (the caller's own policies decide). A session's new SOP row gets a fresh id (the guard), and no session changes an id | — | the same file, NOT applied |
| `public.sop_private_photo_readable(name)`, `sop_private_photo_upload_allowed(name)` | the bucket sop-photos-private: `<doc>/<name>.jpg` read by the document's readers, uploaded by its writers; `staging/<uid>/...` read by that uploader, owner, admin, uploaded by that uploader when they write SOPs or manuals; no update, no delete | — | the same file, NOT applied |
...
| `seesCost(profile)` (`src/lib/cost-access.ts`) | owner, admin; an editor with `profile.sees_cost` (the เห็นต้นทุน switch, read in `getCurrentProfile`); nobody else. THE rule for showing cost on a screen (queue item 56): the recipe pages, the ingredients page, the Star-to-Dog sort. `editAccess` still decides who EDITS; it no longer decides who sees cost |

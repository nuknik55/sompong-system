...
| file | ran | effect |
|---|---|---|
| `sop_visibility_and_editor_cost_switch_migration.sql` | 2026-09-26 | **Queue items 55 and 56, and three views made read-only.** A. Per-SOP "who can see": `menu_sops.visibility` ('all' default, or 'chosen') and `menu_sop_viewers`; one rule (`sop_row_visible` on the SOP's own row, `can_see_sop` by id) behind RESTRICTIVE policies on the SOP, its steps and its notes; only owner and admin set who sees (trigger; `sop_set_visibility` the one save) or delete a restricted SOP; no session moves a step, note or SOP; an SOP request about an SOP its sender may not see cannot be filed, and its author stops reading it. B. The per-editor เห็นต้นทุน switch: `profile_cost_access` (the owner turns it on, editors only; any role change clears it) and `can_see_cost()`, which decides the view `ingredient_costs`, the price-history and POS-receipt reads and `prep_unit_costs()` (replaced, md5-checked first); an old request that carries prices is read only by those who see cost. C. The views `ingredient_costs`, `employee_pay` and `catering_staff_options` read-only. Run by Nik in the SQL editor as committed in `e6014d1`: **109 rows, every line ok, ending "row count verified: 108 evidence rows emitted, as expected (this line makes 109)"**, first run. **Its survey (row 11) CONFIRMED THE HOLE the review had found:** signed-in accounts held INSERT, UPDATE and DELETE on all three views (Supabase's default privileges), so an editor could set a purchase price through `ingredient_costs`, past the ingredients write policy and the approval queue; the views are SELECT-only since. **Never used:** no price-history row was ever written by an editor account (the only way such a write shows; read-only, service key, 2026-09-26: 0 rows; the newest price change of all is 2026-09-21, before the view existed). The switch is ON for ธีรวัฒน์ and เวช. The code followed the same day: `b1a66aa` (the team page's switch, the SOP setting and its lock, the cost screens, the approval checks). **Since then:** a new view or table gets `REVOKE ALL ... FROM PUBLIC, anon, authenticated` before its grants (AGENTS.md, "A view is written through as its OWNER"). |
| `security_fixes_and_menu_save_lock_migration.sql` | 2026-09-26 | **The security round of 2026-09-25/26.** Queue item 48 (the menu page's save locks the booking row first); `handle_new_user` makes a profile only for `<username>@staff.local` logins; every read policy that asked only "signed in" asks for a profile (`public.current_role() IS NOT NULL`); SOP writes owner, admin and editor (they were open to every signed-in account); purchase-price history read owner/admin/editor; `suppliers` and `pos_price_aliases` owner/admin (the live policies printed, then replaced); `templates` and `template_items` written by owner/admin/editor; anon may not execute the seven functions that write; two SECURITY DEFINER guards get a search_path; `sop-photos` takes JPEG up to 2 MB; **salaries owner and hr only** (the four pay columns of `employees` closed to every signed-in account by column grants, served by the view `employee_pay`); **staff see no purchase prices** (the three cost columns of `ingredients` closed the same way, served by the view `ingredient_costs` to owner/admin/editor; `prep_unit_costs()` and `pos_receipt_deliveries` drop staff). Three replaced functions, each md5-checked against its live body first. Run by Nik in the SQL editor as committed in `e9da729`: **124 rows, every line ok, ending "row count verified: 123 evidence rows emitted, as expected (this line makes 124)"**, the second version's first run (the first version, `03a3a17`, stopped at S2 the same day on an empty `pos_price_aliases`; nothing applied). Its survey read **1 login with no profile** (see "Public sign-up, and the two outside logins"). **Nik's check after the run:** one SOP photo and one maintenance photo uploaded, both worked under the JPEG-only 2 MB rule. The other after-run checks listed with the file (hr sees salaries, admin none; staff see no prices; an editor's SOP saves; the team page makes a login that gets its role) were not reported. The code: `c88c44a` (salaries) and `d9829c4` (ingredient costs) were pushed first and now read through the views; `6f84f2b` (templates written on the user's own session) was pushed after the result. **Since then:** a new column on `employees` or `ingredients` needs its own `GRANT SELECT (col) ... TO authenticated`, or every read naming it fails with 42501; and do not roll Vercel back past `d9829c4`: older code selects the closed columns and its HR and ingredient pages fail. |
| `catering_typed_dishes_per_head_and_maintenance_migration.sql` | 2026-09-25 | **Typed dishes, per-head sets and cancellable แจ้งซ่อม.** A. A dish row of a set or a booking's own menu is a menu dish OR a typed name (`dish_name`, `linked_menu_id`; CHECKs: exactly one kind, a link only on a typed dish, 1-120 characters with nothing that prints blank, one name per set or line); owner and admin may link a typed dish to a real menu for its cost (a trigger checks the link, not a foreign key, so PostgREST's `menus(name)` reads stay unambiguous; deleting a menu clears its links); sales writes a booking's menu only through `catering_save_typed_dishes`, typed dishes only. `catering_copy_set_menu`, `catering_save_event_menus` and `catering_save_set_draft` replaced, each checked first against its live body by md5. B. `per_head` on `catering_set_menus` and `catering_event_menus` (a line takes its set's flag when made and keeps it). C. `maintenance_reports` gains status `cancelled` with `cancelled_by`, `cancelled_by_name`, `cancelled_at`, `cancel_note`; `maint_create/edit/take/done/cancel` (SECURITY DEFINER) are the only writes; `maint_insert` and `maint_update` dropped, table writes revoked from anon and authenticated (the service key keeps its own). Run by Nik in the SQL editor as committed in `224848f`: **83 rows, every line ok, ending "row count verified: 82 evidence rows emitted, as expected (this line makes 83)"**, first run. Its "before" line read bookings 6, sets 7 (dish rows 50), booking lines 9 (copied dishes 37), charges 31, แจ้งซ่อม 7 (new 1, in progress 1, done 5). The code followed the same day: `06ea8df` (แจ้งซ่อม) and `23af660` (typed dishes and per-head sets). **Since then:** do not roll Vercel back past `23af660` once a typed dish or a per-head set exists; fix forward. |
| `catering_event_sheet_and_set_drafts_migration.sql` | 2026-09-25 | **Item 39's event-details sheet, the image library, the free mark and set-menu drafts.** Four tables (the terms and image libraries, a booking's picks of each), `catering_events.sheet_notes`, `catering_event_charges.is_free`, `catering_set_menus.is_draft` (drafts hidden from sales), `catering_save_event_sheet`, `catering_save_set_draft`, `catering_make_set_real`, the library upload rule, and `catering_save_booking_prices` and `catering_copy_set_menu` replaced (Step 0 checked both live bodies by md5 first). The private bucket `catering-details` holds the image library only: owner and admin upload, owner, admin and sales read, no update or delete, at most 100 current files (live `storage.objects` is versioned: archived versions and delete markers do not count). Dropped sop-photos' broad read "sop photos public read". Run by Nik in the SQL editor as committed in `41d97c7`: **117 rows, every line ok, ending "row count verified: 116 evidence rows emitted, as expected (this line makes 117)"**, the second version's first run (the first version, `5811ec3`, stopped at S21 on the versioned table, nothing applied). Its "before" line read bookings 6, sets 7 (dish rows 50), booking lines 9 (copied dishes 37), charges 31, history 93, files in catering-details 0, in sop-photos 411. **Nik's check after the run:** one SOP photo and one maintenance photo uploaded, both worked, so dropping the broad read broke nothing (the file's header keeps the one line that would restore it). The code followed the same day: `d73fe9e` (drafts), `90c0101` (the free mark), `85736a4` and `89ac246` (the sheet and the image library). |
...
48. **The menu page's save does not wait for the booking screen's**
    (review of item 47, 2026-09-21). **FIXED 2026-09-26, applied:**
...
55. **Per-SOP "who can see" (Nik, 2026-09-26).** **LIVE 2026-09-26:**
    `sop_visibility_and_editor_cost_switch_migration.sql` applied (see
    "Applied since"), the code `b1a66aa`. Each SOP is open
    ...
    - **Prep SOPs: Nik chose A (2026-09-26)** — attached to a prep recipe,
      read by those who can see the prep and never by staff, written by
      owner, admin and editors who can see the prep; see the next items.
      What follows was the finding and the proposal.
      An SOP cannot belong to a prep recipe today:
    ...
    - **OPEN RISK: the photos. Nik chose option 1 (2026-09-26)**, a private
      bucket, as its own round. `sop-photos` is a PUBLIC bucket: a step
...
56. **The per-editor เห็นต้นทุน switch (Nik, 2026-09-26).** **LIVE 2026-09-26**
    (the same file and code as item 55). On the team page, per
...
pages after the quotation, and alone from `[id]/details/print`), live
since 2026-09-25 (`catering_event_sheet_and_set_drafts_migration.sql`
applied, the code in `85736a4` and `89ac246`). **The print
...
### Public sign-up, and the two outside logins (2026-09-25/26)

**Found 2026-09-25:** public sign-up was ON (`/auth/v1/settings` read
`disable_signup: false`), and `handle_new_user` gave every new login a
`staff` profile, so anyone who found the project's address could have made
an account and read what staff read. Two logins from outside the team
existed, both made 2026-06-27, neither with a profile.

- **Sign-up is OFF:** Nik switched "Allow new users to sign up" off on
  2026-09-26; read back the same day with the public key
  (`disable_signup: true`). Since the security migration (applied
  2026-09-26) `handle_new_user` also makes a profile only for a
  `<username>@staff.local` login, the team page's.
- **One outside login was deleted** by Nik on 2026-09-26.
- **The other is KEPT, by Nik's decision (2026-09-26): not deleted, not
  banned.** It could not be deleted: it is `changed_by` on 87
  `ingredient_price_history` rows and 40 `recipe_item_history` rows, and
  those foreign keys to `auth.users` have no ON DELETE action, so the
  delete fails with "Database error deleting user" (read-only, service
  key, 2026-09-26; its email was not printed). It has no profile, and the
  migration's survey counted exactly one such login. Every read policy now
  asks `public.current_role() IS NOT NULL`, so a session of it gets
  nothing an anonymous visitor would not. **If it is ever to go:** ban it
  through the admin API, or clear or reassign those 127 `changed_by`
  values (or give the two foreign keys ON DELETE SET NULL) and then delete
  it.

...
| `catering_event_sheet_and_set_drafts_migration.sql` (SECOND VERSION, replacing `5811ec3`'s under the same name) | Nik. Expected: **117 rows**, ending "row count verified: 116 evidence rows emitted, as expected (this line makes 117)". Run it while nobody is uploading a SOP or maintenance photo. **The first version was run once and stopped, nothing applied** (2026-09-24, S21: it simulated the storage API's upsert with ON CONFLICT (bucket_id, name), which the live storage.objects has no matching constraint for; checked read-only afterwards: no new table, column, function or bucket). The second simulates no storage internals, keeps the bucket for the IMAGE LIBRARY only (images for one booking are picked on the print page and never uploaded), drops sop-photos' broad read policy "sop photos public read", and stops before replacing `catering_save_booking_prices` or `catering_copy_set_menu` if either live body differs from the one it copied (md5; both matched Nik's read-only query, 2026-09-25). Live `storage.objects` is VERSIONED (that query: `version`, `archived_at`, `is_versioned`, `is_delete_marker`, partial unique indexes and no plain unique key on (bucket_id, name), which is what the first version's ON CONFLICT hit): the 100-file cap counts current files only, an upload may not be a delete marker or an archived version, and Step 0 stops with nothing changed if those two columns are not there as the query showed them. A library image's file is fixed once saved (a trigger); the 100-file cap is soft (two uploads at 99 can both pass) and counts current files whose image was deleted, which no app role can remove (only the service key, with Nik's permission). | Nothing in production uses it. The code that does waits on the local branch `event-sheet-set-drafts` and is pushed only after it has run. What SQL cannot test (the file's header): the storage API itself (type and size limits, signed links, signed upload links, copies between buckets, a file's real content, how it decides a permission), the browser's resize, the print page's images, a real anonymous HTTP request. After it runs: one library upload, one sheet print with a library image, and one SOP and one maintenance photo upload (sop-photos without its broad read; SQL cannot show whether the storage server needs it, and the file's header holds the one line that puts it back). |
...
**Nik's check after `catering_event_sheet_and_set_drafts_migration.sql` runs:**

- Upload one SOP photo and one maintenance photo through the app.
- If either upload fails, put back the read policy the file dropped, with
  the one-line CREATE POLICY from the file's header:

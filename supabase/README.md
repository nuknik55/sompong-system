...
|---|---|---|
| `catering_typed_dishes_per_head_and_maintenance_migration.sql` | Nik. Expected: **83 rows**, ending "row count verified: 82 evidence rows emitted, as expected (this line makes 83)". Run it while nobody saves a booking's menu or a set menu or files a แจ้งซ่อม. **A. Typed dishes** (dishes not in the menu list): a dish row of a set or a booking's own menu is a menu dish OR a typed name (CHECKs; 1-120 characters, nothing that prints blank; one name per set or line); owner and admin may link a typed dish to a real menu for its cost (a trigger, not a foreign key, checks the link; a menu deleted clears its links); sales writes a booking's menu only through `catering_save_typed_dishes` (typed dishes only; cancelled and cost-locked bookings refused for everyone; every save rewrites the typed rows so an older screen is refused). `catering_copy_set_menu`, `catering_save_event_menus` and `catering_save_set_draft` are replaced, each checked first against its live body (md5). **B. Per head**: `per_head` on sets and booking lines (a line takes its set's flag when made and keeps it). **C. แจ้งซ่อม**: status `cancelled` with who, when and a note; five SECURITY DEFINER functions are the only writes (`maint_create/edit/take/done/cancel`); `maint_insert` and `maint_update` dropped and table writes revoked from anon and authenticated (the service key keeps its own, which the code live when it runs uses). | **Deploy order: this file first, then the code** (local branch `typed-dishes-per-head-maint`). The code live when it runs keeps working: it writes แจ้งซ่อม with the service key, and no second foreign key to `menus` is added, so its `menus(name)` reads stay unambiguous. Once a typed dish or a per-head set exists, do not roll the code back past it: fix forward. **After it runs:** push the code; then file a test แจ้งซ่อม with a photo and cancel it (as the one who filed it), and have an editor cancel another; add a typed dish to a booking as sales; open the cost page of a booking with a typed dish and see the warning. |
